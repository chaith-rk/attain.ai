...
| Action | What user actually did | Only when user reports | LLM based on user input (confirmed) |
| Notes | Intent vs Action comparison | After Action is filled | LLM auto-generates (brief, supportive) |

### Key Rules
1. Intent = planning. Filled before or on the day.
2. Action = reality. ONLY filled when user explicitly reports.
3. Notes = reflection. Auto-generated by LLM after Action exists (<=200 chars, supportive, no judgment).
...
| Report action | "I did my run today" | Propose Action for today, confirm, then apply |
...
- Shows "Intent" or "Action" and "Date" on two lines
